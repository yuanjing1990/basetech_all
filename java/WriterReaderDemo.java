...
public class WriterReaderDemo {
    /*
     * 按处理对象可以分为：字节流和字符流 字节流的基类为InputStream和OutputStream，一次处理一个字节
     * 字符流的基类为Reader和Writer，一次处理一个字符（2个字节）
     *
     * 按数据流向可以分为：输入流和输出流 输入流的基类为InputStream和Reader 输出流的基类为OutputStream和Writer
     *
     * 直接与数据源对接的为节点流，主要有： FileInputStream,FileOutputStream,FileReader,FileWriter
     * 处理文件
     * ByteArrayInputStream,ByteArrayOutputStream,CharArrayReader,CharArrayWriter
     * 处理数组 StringReader,StringWriter 处理字符串
